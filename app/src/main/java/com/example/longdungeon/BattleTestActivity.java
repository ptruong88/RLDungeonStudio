...
public class BattleTestActivity extends ActionBarActivity implements
	...
	private void setUpStringForListView() {
		...
		listAttack[0] = generateSkillDescription("Light Attack ", baseDamage,
				baseStm, "STM");
		listAttack[1] = generateSkillDescription("Medium Attack ",
				(int) (baseDamage * mediumRatio),
				(int) (baseStm * mediumRatio), "STM");
		listAttack[2] = generateSkillDescription("Heavy Attack ", baseDamage
				* heavyRatio, baseStm * heavyRatio, "STM");
		...
		listMagic[0] = generateSkillDescription("Fire Magic ", baseDamage,
				baseMana, "MANA");
		listMagic[1] = generateSkillDescription("Ice Magic ",
				(int) (baseDamage * mediumRatio),
				(int) (baseMana * mediumRatio), "MANA");
		listMagic[2] = generateSkillDescription("Lightning Magic ", baseDamage
				* heavyRatio, baseMana * heavyRatio, "MANA");
	...
	// returns string that describes skills, used when attack or magic buttons
	// are pressed
	// example: "Ice Magic 10DMG / 7MANA"
	private String generateSkillDescription(String skillName, int damage,
			int baseCost, String resourceBeingSpent) {
	...
	private void playerMagicAttackAnimation(final ImageView imageMagic) {
		 ...
		 * getApplicationContext(), R.anim.move_magic_player); // set animation
		 * listener animMove.setAnimationListener(this);
		 * 
	...
	private void playerPhysicalAttackAnimation(final ImageView imageEffect) {
		 ...
		 * 
		 * // set animation listener animMove.setAnimationListener(this);
		 * 
		 * final Animation animShake = AnimationUtils.loadAnimation(
		 * getApplicationContext(), R.anim.shake); // set animation listener
		...
		/*
		 * imgPlayer.startAnimation(animMove); // now start walk
		 * AnimationDrawable theKnightAnimation = (AnimationDrawable) imgPlayer
		 * .getBackground(); theKnightAnimation.start();
		 */
	...
	private void attackEnemyFinishMove(int damage, String attackDescription,
		...
		Toast.makeText(getApplicationContext(),
				attackDescription + damage + " damage!", Toast.LENGTH_LONG)
				.show();
		++count;
		System.out.println("___----" + count);
	...
	private void attackEnemyPhysicAnimation(final ImageView imageEffect) {
		...
		/*
		 * Animation animMove = AnimationUtils.loadAnimation(
		 * getApplicationContext(), R.anim.move_right); // set animation
		 * listener animMove.setAnimationListener(this);
		 * 
		 ...
		 * getApplicationContext(), R.anim.shake);
		 * 
		 * // set animation listener animShake.setAnimationListener(this);
		 */
